<script lang="ts">
	...
	const maxAbsValue = $derived.by(() => {
		const maturedValues = data.weeks.map((week) => week.anki_matured_passages);
		const lostValues = data.weeks.map((week) => week.anki_lost_passages);
	...
	const maxAbsCumulative = $derived.by(() => {
		const cumulativeValues = data.weeks.map((week) => week.anki_cumulative_passages);
	...
	const chartData = $derived({
		...
		datasets: [
			...
			{
				label: 'Matured',
				data: data.weeks.map((week) => week.anki_matured_passages),
			...
			{
				label: 'Lost',
				data: data.weeks.map((week) => -week.anki_lost_passages),
			...
			// Line dataset: Cumulative passages (right axis)
			{
				label: 'Cumulative',
				type: 'line' as const,
				data: data.weeks.map((week) => week.anki_cumulative_passages),
